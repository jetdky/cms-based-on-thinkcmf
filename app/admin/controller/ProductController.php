...
//        halt($product);
    ...
    public function saveMove()
    {
        ...
        Db::name('product')->whereIn('id', $id_array)->update(['cid' => $data['cid'], 'lang' => $data['lang']]);
        return true;
    }
     ...
     * @param
     */
    public function saveCopy(ProductModel $productModel, ImgContentModel $imgContentModel,TagContentModel $tagContentModel)
    {
        $data = $this->request->param();
        $id_array = explode(',', $data['id']);
        $productData = $productModel->whereIn('id', $id_array)->select()->toArray();
        foreach ($productData as $k => $v) {
            ...
            $v['cid'] = $data['cid'];
            $v['lang'] = $data['lang'];
            $productModel->isUpdate(false)->data($v,true)->save($v);
            $id = $productModel->id;
            if (!empty($imgs)) {
                $b = [];
